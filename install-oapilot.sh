...
create_scripts() {
    ...
    # Copy the full oapilot script to ~/.local/bin and update OAPILOT_HOME
    cp "$INSTALL_DIR/oapilot" "$HOME/.local/bin/oapilot"

    # Set OAPILOT_HOME environment variable in the global command
    sed -i "1a\\
# Auto-generated by OAPilot installer\\
export OAPILOT_HOME=\"$INSTALL_DIR\"" "$HOME/.local/bin/oapilot"
